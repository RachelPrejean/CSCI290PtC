...
public class RaggedArrayList<E> implements Iterable<E> {
    ...
    public ListLoc findFront(E item) {
        ...
        for(int row=0; row < l1Array.length; row++){
            L2Array l2Array = (L2Array) l1Array[row];
            for (int col=0; col< l2Array.numUsed;col++){
                if(l2Array.items[col].equals(item)){
                    return new ListLoc(row,col);
                }
            }
    ...
    public ListLoc findEnd(E item) {
        // TO DO in part 3
        for(int row=l1Array.length - 1; row >= 0; row--){
            L2Array l2Array = (L2Array) l1Array[row];
            for (int col=l2Array.numUsed - 1; col >= 0; col--){
                if(l2Array.items[col].equals(item)){
                    return new ListLoc(row,col); // should return last index of the item searched 
                }
            }
        }
        return null; // when finished should return: new ListLoc(l1,l2);
    ...
    public boolean add(E item) {
        ...
        ListLoc end =  this.findEnd(item);
        l1Array = Arrays.copyOf(l1Array, l1Array.length + 1);
        l1Array[end]= item;
