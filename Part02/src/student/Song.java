...
public class Song implements Comparable {
    ...
    public int compareTo(Object that) {
        ...
        if (that == null){
            ...
            Song toCompare = (Song) that;
            int authorCompare = this.artist.compareToIgnoreCase(toCompare.getArtist());
            if (authorCompare < 0 ) {return before;}
            else if (authorCompare > 0) {return after;}
            else {
                int titleCompare = this.title.compareToIgnoreCase(toCompare.getTitle());
                if (titleCompare < 0) {return before;}
                else {return after;}
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("testing compareArtist");
        CmpArtist compareArtists = new CmpArtist();
        if (compareArtists.compare(s1, s2) <= 0){
            System.err.printf("FAILED: %s should be greater than %s",
                              s1.getArtist(), s2.getArtist());
        }
        if (compareArtists.compare(s2, s1) >= 0){
            System.err.printf("FAILED: %s should be less than %s",
                              s2.getArtist(), s1.getArtist());
        }
        if (compareArtists.compare(s1, s3) != 0){
            System.err.printf("FAILED: %s should be equal to %s",
                              s1.getArtist(), s3.getArtist());
        }
        if (compareArtists.compare(s3, s1) != 0){
            System.err.printf("FAILED: %s should be equal to %s",
                              s3.getArtist(), s1.getArtist());
        }
        if (compareArtists.compare(s1, s1) != 0){
            System.err.printf("FAILED: %s should be equal to %s",
                              s1.getArtist(), s1.getArtist());
        }
    }
