 ...
 * data type (object)
 */
public class RaggedArrayList_mine<E> implements Iterable<E> {
     ...
     */
    public RaggedArrayList_mine(Comparator<E> c) {
        size = 0;
    ...
    public ListLoc findFront(E item) {
        ...
        if (l1Array[l1] == null || comp.compare(((L2Array) l1Array[l1]).items[0], item) > 0){
            ...
            // Iterate through the list to find the first index that is less
              // than the target
            while(((L2Array) l1Array[l1]).items[l2+1] != null && comp.compare(((L2Array)l1Array[l1]).items[l2], item) < 0){
                l2++;
            }
     ...
     * @return the sublist
     */
    public RaggedArrayList_mine<E> subList(E fromElement, E toElement) {
        // TO DO in part 5 and NOT BEFORE

        RaggedArrayList_mine<E> result = new RaggedArrayList_mine<E>(comp);
        return result;
    }
